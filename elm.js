...
Elm.DragDropList = Elm.DragDropList || {};
Elm.DragDropList.make = function (_elm) {
   "use strict";
   _elm.DragDropList = _elm.DragDropList || {};
   if (_elm.DragDropList.values)
   return _elm.DragDropList.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "DragDropList",
   $Basics = Elm.Basics.make(_elm),
            ...
            return function () {
                 ...
                 {case "_Tuple2":
                    return {ctor: "_Tuple2"
                           ...
                           ,_1: _v1._1 - _v0._1};}
                 _U.badCase($moduleName,
                 "on line 89, column 25 to 39");
              }();}
         _U.badCase($moduleName,
         "on line 89, column 25 to 39");
      }();
   });
   var dragDropUpdate = function (m) {
      return function () {
         var moo = A2($Debug.watch,
         "dragDrop",
         m);
         return function () {
            switch (moo.ctor)
            {case "Just":
               switch (moo._0.ctor)
                 {case "_Tuple2":
                    switch (moo._0._1.ctor)
                      {case "MoveBy":
                         switch (moo._0._1._0.ctor)
                           {case "_Tuple2":
                              return moveBy({ctor: "_Tuple2"
                                            ,_0: moo._0._1._0._0
                                            ,_1: 0 - moo._0._1._0._1});}
                           break;
                         case "Release":
   ...
   var sampleGap = $Graphics$Element.color($Color.red)(A2($Graphics$Element.spacer,
   100,
   10));
   var sampleElement = $Graphics$Element.color($Color.yellow)(A4($Graphics$Element.container,
   200,
   100,
   $Graphics$Element.middle,
   $Text.plainText("What")));
   var sampleList = A2($List.repeat,
   7,
   sampleElement);
   var listWithFloater = F3(function (list,
   gapId,
   _v19) {
      return function () {
         switch (_v19.ctor)
         {case "_Tuple2":
            return function () {
                 ...
                 $Graphics$Element.absolute(_v19._0),
                 $Graphics$Element.absolute(_v19._1));
                 var _raw = A2($List.drop,
                 gapId,
                 list),
                 ...
                 gapElement = $._0,
                 afterGap = $._1;
                 var $ = $Graphics$Element.sizeOf(gapElement),
                 x = $._0,
                 y = $._1;
                 var floater = A4($Graphics$Element.container,
                 x + _v19._0,
                 y + _v19._1,
                 pos,
                 gapElement);
                 var beforeGap = A2($List.take,
                 gapId,
                 list);
                 return A2($Basics._op["++"],
                 beforeGap,
                 A2($Basics._op["++"],
                 _L.fromArray([floater]),
                 afterGap));
              }();}
         _U.badCase($moduleName,
   ...
   var sameSizedSpacer = function (element) {
      return function () {
         var $ = $Graphics$Element.sizeOf(element),
         x = $._0,
         y = $._1;
   ...
   var dragPadElement = $Graphics$Element.color($Color.black)(A2($Graphics$Element.spacer,
   30,
   40));
   var gapElement = function (model) {
      return function () {
         var maxWidth = A3($List.foldr,
         function (el) {
            return $Basics.max($Graphics$Element.widthOf(el));
         },
         0,
         model.list);
         var $ = $Graphics$Element.sizeOf(model.gap),
         gapW = $._0,
         gapH = $._1;
         var $ = $Graphics$Element.sizeOf(dragPadElement),
         dragW = $._0,
         dragH = $._1;
         return A3($Graphics$Element.container,
         maxWidth + dragW,
         gapH,
         $Graphics$Element.midRight)(A4($Graphics$Element.container,
         maxWidth,
         gapH,
         $Graphics$Element.middle,
         model.gap));
   ...
   var Dragging = F2(function (a,
   b) {
      return {ctor: "Dragging"
             ,_0: a
             ,_1: b};
   });
   var dragStatus = F2(function (dragDrop,
   movement) {
      return function () {
         switch (dragDrop.ctor)
         {case "Just":
            switch (dragDrop._0.ctor)
              {case "_Tuple2":
                 switch (dragDrop._0._1.ctor)
                   {case "Release":
                      return NotDragging;}
                   return $Debug.watch("drag")(A2(Dragging,
                   dragDrop._0._0,
                   movement));}
              break;
            case "Nothing":
   ...
   var hover = $Signal.channel($Maybe.Nothing);
   var hoverWatch = A2($Debug.watch,
   "hover",
   $Signal.subscribe(hover));
   var dragPad = function (id) {
      return function () {
         var action = function (hovering) {
            return $Debug.watch("hover")(function () {
               switch (hovering)
               {case false:
                  return $Maybe.Nothing;
                  case true:
         ...
         function ($) {
            return $Signal.send(hover)(action($));
         },
         dragPadElement);
      }();
   };
   var listWithDragpads = function (list) {
      return function () {
         var idList = A2($List.indexedMap,
         F2(function (v0,v1) {
            return {ctor: "_Tuple2"
                   ,_0: v0
                   ,_1: v1};
         ...
         dragW = $._0,
         dragH = $._1;
         var positionedPad = F2(function (id,
         height) {
            return A3($Graphics$Element.container,
            dragW,
            height,
            $Graphics$Element.middle)(dragPad(id));
         });
         var makeIdPad = function (_v28) {
            return function () {
               switch (_v28.ctor)
               {case "_Tuple2":
                  return _L.fromArray([positionedPad(_v28._0)($Graphics$Element.heightOf(_v28._1))
                                      ,_v28._1]);}
               _U.badCase($moduleName,
               "on line 38, column 30 to 64");
            }();
         };
         return A2($List.map,
         makeIdPad,
   ...
   var listElements = function (model) {
      return function () {
         var $ = $Graphics$Element.sizeOf(dragPadElement),
         dragW = $._0,
         dragH = $._1;
         return $List.map($Graphics$Element.flow($Graphics$Element.right))(listWithDragpads(model.list));
      }();
   };
   var view = F2(function (model,
   dragging) {
      return function () {
         var pureElements = listElements(model);
         var elements = function () {
            switch (dragging.ctor)
            {case "Dragging":
               switch (dragging._1.ctor)
                 {case "_Tuple2":
                    return A3(listWithFloater,
                      pureElements,
                      dragging._0,
                      {ctor: "_Tuple2"
                      ,_0: dragging._1._0
   ...
   var main = function () {
      var see = view({_: {}
                     ,gap: sampleGap
                     ,list: sampleList});
      var track = A2($DragAndDrop.trackMany,
      $Maybe.Nothing,
      $Signal.subscribe(hover));
      var totalDrag = A3($Signal.foldp,
      dragDropUpdate,
      {ctor: "_Tuple2",_0: 0,_1: 0},
      track);
      var drag = A3($Signal.map2,
      dragStatus,
      track,
      totalDrag);
      return A2($Signal.map,see,drag);
   }();
   _elm.DragDropList.values = {_op: _op
                              ,main: main};
   return _elm.DragDropList.values;
};
